...
public:
     ...
     * 说明：
     * - 本接口会优先获取“透明背景的纯光标图像”；
     * - 对部分依赖 AND/XOR 掩码的单色系统光标，会返回“规范化后的纯光标图像”：可直接确定的黑/白像素按其原义输出，透明区域保持透明，依赖背景反相显示的像素会根据当前背景整体亮度自适应输出为黑或白；
     ...
     * - 对部分单色系统光标，主路径返回的是“规范化后的纯光标图像”，其中依赖背景反相显示的像素会根据当前背景整体亮度自适应输出为黑或白；
