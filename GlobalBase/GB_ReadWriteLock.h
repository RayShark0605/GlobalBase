...
/*
    ...
    - 本实现采用“写优先（writer-preference）”策略：
      ...
      代价是：在持续写压力下，读者可能出现饥饿。

    限制：
    - 非递归：同一线程重复持有同一把锁（尤其写锁）再去加锁可能死锁。
    - 不支持“读锁原子升级为写锁”：如需升级必须释放读锁再抢写锁，并在抢到写锁后重检条件。
    - UnlockShared() 必须与成功的 LockShared()/TryLockShared()/TryLockSharedFor() 配对。
    - Unlock() 必须与成功的 Lock()/TryLock()/TryLockFor() 配对。
*/
...
public:
    ...
    GB_ReadWriteLock(GB_ReadWriteLock&& other) = delete;
    GB_ReadWriteLock& operator=(GB_ReadWriteLock&& other) = delete;
...
bool GB_ReadWriteLock::TryLockSharedFor(const std::chrono::duration<Rep, Period>& timeout)
{
    ...
    if (!readersCondition_.wait_until(lockGuard, deadline, [this]() { return CanLockSharedUnlocked(); }))
    {
        return false;
    }
...
bool GB_ReadWriteLock::TryLockFor(const std::chrono::duration<Rep, Period>& timeout)
{
    ...
    try
    {
        if (!writersCondition_.wait_until(lockGuard, deadline, [this]() { return CanLockUnlocked(); }))
        {
            waitingWriters_--;
